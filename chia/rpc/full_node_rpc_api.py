...
class FullNodeRpcApi:
    ...
    async def get_coin_records_by_hints(self, request: Dict[str, Any]) -> EndpointResult:
        """
        Retrieves coins for hints, by default returns unspent coins.
        """
        ...
        hints: List[bytes32] = [bytes32.from_hexstr(hint) for hint in request["hints"]]

        names: List[bytes32] = await self.service.hint_store.get_coins_ids_by_hints(hints)
